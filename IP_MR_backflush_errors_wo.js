 ...
 * Set the RETRY COMPLETION checkbox as true for all the valid AM Completion Errors, trying to solve the backflush issues after the user's actions.
 *
 * Version	    Date			    Author				 Project/Ticket Number		           Remarks
 * 1.0		  15 Apr 2019	 	Ivan Posadas		        ENHC-0000000		             Initial Release
 */
define(["N/log", "N/search", "N/runtime", "N/record"], function(
  ...
  function getInputData() {
    var stSearch_NAME = objCurrentScript.getParameter(
      "custscript_mr_backflush_am_search_name"
    );
    var arrSearchResults = NSUtil.search(null, stSearch_NAME, null, null);
    var arrAMCompErrors = [];
    arrSearchResults.forEach(function(objResult) {
      var intAMCompError_ID = Number(objResult.getValue(objResult.columns[0]));
      if (
        !isNaN(intAMCompError_ID) &&
        arrAMCompErrors.indexOf(intAMCompError_ID) < 0
      ) {
        arrAMCompErrors.push(intAMCompError_ID);
      }
    });
    log.audit({
      title: "ACE to process",
      details: arrAMCompErrors.length
    });
    log.audit({
      title: "input:GU",
  ...
  function map(context) {
    var stAMCompErrorRecName = objCurrentScript.getParameter(
      "custscript_mr_backflush_rec_name"
    );
    log.debug({
      title: "map",
      details: "Start"
    ...
    var recAMCompError = record.load({
      type: stAMCompErrorRecName,
      id: context.value
    });

    recAMCompError.setValue({
      fieldId: "custrecord_am_wocomperror_forcecomp",
      value: true
    });
